...
public class MethodReflectorImpl<M> implements MethodReflector<M> {
  ...
  public MethodReflectorImpl(Method method) {
    this.method = method;
    this.methodParamGenerics =
        TypeParameterResolver.resolveMethodParamTypes(this.method, this.method.getDeclaringClass());
    this.returnTypeGeneric =
        TypeParameterResolver.resolveReturnType(this.method, this.method.getDeclaringClass());
  ...
  public ParameterReflector parameterReflector(int index) {
    return this.parameterReflectors.stream()
        ...
        .findFirst()
        .orElseThrow(() -> new ReflectException(String.format("不存在一个参数索引为[%d]的参数", index)));
  }
  ...
  public boolean isThisMethodReflector(String methodName, Class<?>... methodParameterTypes) {
    return this.method.getName().equals(methodName)
        && Arrays.equals(this.method.getParameterTypes(), methodParameterTypes);
  }
  ...
  private List<ParameterReflector> initParameterReflectors() {
    ...
    for (int i = 0; i < parameters.length; i++) {
      parameterReflectorList.add(
          new ParameterReflectorImpl(parameters[i], i, this.methodParamGenerics[i]));
    }

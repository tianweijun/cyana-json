...
public class ConstructorReflectorImpl<T> implements ConstructorReflector<T> {
  ...
  public ConstructorReflectorImpl(Constructor<T> constructor) {
    ...
    this.constructorParamGenerics =
        TypeParameterResolver.resolveConstructorParamTypes(
            this.constructor, this.constructor.getDeclaringClass());
  ...
  private List<ParameterReflector> initParameterReflectors() {
    ...
    for (int i = 0; i < parameters.length; i++) {
      parameterReflectorList.add(
          new ParameterReflectorImpl(parameters[i], i, this.constructorParamGenerics[i]));
    }
